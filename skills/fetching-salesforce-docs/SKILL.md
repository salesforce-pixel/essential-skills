...
<!--
Changelog
1.8 — Documented host prerequisites: README now has a Requirements section with
      per-OS Python/pip/venv install commands and a verify step. The bootstrap
      runs an actionable preflight before creating the venv — clear message
      (naming python3-venv on Debian) when Python is too old or lacks
      ensurepip/venv, instead of a generic provisioning failure.
