...
```sfdocs-code {"lang":"agentscript", "title": "Example: No Specific Reasoning Instructions"}
reasoning:
    instructions: ->
...
```sfdocs-code {"lang":"agentscript", "title": "Example: Additional Context For Using the capture_order_info Tool "}
reasoning:
    instructions: ->
...
Use `available when` to define the conditions that must exist for the LLM to use the tool. The clause after `available when` must be a valid [conditional expression](ascript-ref-expressions.md) built from [supported operators](ascript-ref-operators.md).

```sfdocs-code {"lang":"agentscript", "title": "Example: Available When"}
reasoning:
    actions:
...
```sfdocs-code {"lang":"agentscript", "title": "Example: Using Subagents as Tools"}
reasoning:
    actions:
